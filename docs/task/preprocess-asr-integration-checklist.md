...
- [ ] 在 `internal/ffmpeg` 增加独立的音频选区导出函数，不复用视频 clean shot 裁切参数
- [ ] 在 `localagent.Processor` 增加可测试的 `ExtractAudio` 方法
- [ ] 输入使用参数数组调用 `ffmpeg`，不得拼接 shell 命令，确保中文和空格路径可用
- [ ] 导出参数固定为第一条音频流、`16000Hz`、单声道、`pcm_s16le` WAV
- [ ] 导出范围严格使用当前请求的 `source_in_ms` 和 `source_out_ms`
- [ ] 校验导出 WAV 的实际时长与选区时长一致，允许不超过一个音频采样周期或明确的小误差
- [ ] 输入无音频流时返回明确错误，不生成空 WAV
- [ ] I/O 超出当前工作源时长时直接拒绝，不自动扩展或重置 I/O
- [ ] 音频临时文件使用每次请求独立目录，成功、失败和取消后都删除
...
- [ ] 新增 `POST /workspace/items/:itemID/transcribe`
- [ ] 请求至少包含当前 `source_in_ms`、`source_out_ms`、`server_base_url` 和 `auth_token`
- [ ] 校验工作区条目存在且当前素材类型为 `talking_head`
- [ ] 校验当前工作源 `probe.has_audio=true`
- [ ] 以请求中的当前 I/O 为准，不能使用导入时默认全片范围替代
- [ ] 转写开始和结束后都不修改 `SourceInMs`、`SourceOutMs`、预览帧或时间轴状态
- [ ] 将临时 WAV 以 multipart 上传到服务端 `/api/preprocess/asr-transcribe`
- [ ] 透传当前登录令牌，并正确处理登录失效、服务端不可用和请求超时
- [ ] 校验服务端回传的 `source_in_ms`、`source_out_ms` 与本次请求一致，防止旧请求结果覆盖新选区

### 2.3 本地结构化草稿

- [ ] 为工作区条目增加结构化 ASR 草稿，至少保存完整文本、句段、对应 I/O 和生成时间
- [ ] 句段在本地保持毫秒整数，不转换成 SRT 文本
- [ ] ASR 草稿时间保持选区相对时间，范围必须位于 `0` 到选区时长之间
- [ ] ASR 结果只更新草稿，不静默覆盖用户已经确认或手工编辑的 `Transcript`
- [ ] 只有本次 I/O 仍与发起请求时一致时才写入草稿
- [ ] 用户之后修改 I/O 时，将旧草稿标记为过期或清除，不能继续当作当前选区结果
- [ ] 工作区状态持久化后重启 local-agent 仍能恢复 ASR 草稿
- [ ] ASR 失败时保留原有人工转写和上一次有效草稿，并返回本次错误
- [ ] 不生成 `.srt` 文件，不创建 clean shot，不触发正式提交

### 2.4 local-agent 自动化测试

- [ ] 为 ffmpeg 音频导出参数增加单元测试，覆盖中文路径和非零 I/O
- [ ] 使用 Processor stub 验证提取范围严格等于请求 I/O
- [ ] 使用 HTTP 测试服务验证上传文件格式、认证头和时间范围字段
- [ ] 覆盖无音频、非法 I/O、ffmpeg 失败、服务端失败、超时和返回范围不匹配
- [ ] 覆盖请求期间用户改变 I/O 时旧结果不得覆盖当前工作区
- [ ] 覆盖已有人工 `Transcript` 不被 ASR 草稿覆盖
- [ ] 覆盖成功、失败和取消后的本地临时文件清理
- [ ] 覆盖 local-agent 重启后结构化草稿恢复
...
- [ ] 直接把上述绝对路径传给 `ExtractAudio`，证明中文路径可用
- [ ] 使用全范围 `I=0ms`、`O=19017ms` 导出 WAV
- [ ] 使用 `ffprobe` 确认 WAV 为 `pcm_s16le`、`16000Hz`、单声道、约 `19.017s`
- [ ] 将该 WAV 经服务端代理发送给 FunASR，返回非空且可读的中文转写
- [ ] 确认所有句段时间均位于 `0..19017ms`
- [ ] 再选择一个非零入点的子区间，确认实际导出时长等于 `O-I`
- [ ] 确认子区间 ASR 句段仍从选区相对 `0ms` 开始，不错误叠加源视频入点
- [ ] 确认在原始视频时间轴回显时使用 `source_in_ms + segment time`
- [ ] 转写前后核对时间轴 I/O 完全不变
- [ ] 核对 local-agent 与服务端均无遗留临时音频
- [ ] 核对服务端没有新增素材、任务、向量或素材级日志记录

### 2.6 第 2 阶段完成标准

- [ ] local-agent 能从任意有效 `talking_head` 当前 I/O 选区导出标准 WAV
- [ ] local-agent 能通过受认证服务端接口取得结构化 FunASR 结果
- [ ] 真实样本能够得到可读转写和有效毫秒级句段
- [ ] ASR 草稿与发起请求时的 I/O 严格绑定，不重置时间轴，也不覆盖人工确认文本
- [ ] 全链路不生成 SRT、不提前提交素材、不遗留临时音频
- [ ] 自动化测试与真实视频验收全部通过后，才进入前端转写弹窗接入阶段
