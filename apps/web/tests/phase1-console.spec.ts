...
test("logs in and renders asset and task status pages", async ({ page }) => {
  ...
  await expect(page.getByText("卖点关联素材")).toBeVisible();
  ...
  await expect(page.getByRole("cell", { name: "可用" }).first()).toBeVisible();
  ...
  await expect(page.getByRole("cell", { name: "特写" }).first()).toBeVisible();
  await expect(page.getByRole("cell", { name: "固定机位" }).first()).toBeVisible();
  ...
  await page.getByTitle("最近更新时间").click();
  ...
  await expect(page.getByText("暂无质量问题")).toBeVisible();
  ...
  await page.getByLabel("画面描述").fill("人工修订后的描述");
  await page.getByLabel("复核备注").fill("编辑已复核");
  await page.getByLabel("质量标签").fill("轻微软焦");
  await page.keyboard.press("Enter");
  await page.getByTestId("save-asset-review").click();
  await expect(page.getByText("人工修订后的描述")).toBeVisible();
  await expect(page.getByText("编辑已复核")).toBeVisible();
  await expect(page.getByText("轻微软焦")).toBeVisible();
  await expect(page.getByText("无", { exact: true }).first()).toBeVisible();
  ...
  await expect(page.getByText("素材抽帧")).toBeVisible();
  await expect(page.getByText("素材分析")).toBeVisible();
  await expect(page.getByText("231 毫秒")).toBeVisible();
  await page.getByTestId("task-filter-type").click();
  await page.getByTitle("素材抽帧").click();
  ...
  await page.getByTestId("task-filter-type").click();
  await page.getByTitle("素材分析").click();
